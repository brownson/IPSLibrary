<?
	 ...
	 * @{
	 *
	 * @file          IPSComponentCam_Instar720pSeries.class.php
	 * @author        bumaas
	 *
	 */

	/**
    * @class IPSComponentCam_Instar720pSeries
    *
    * Definiert ein IPSComponentCam Object, das die Funktionen einer Cam Componente für eine
    * Instar720pSeries Kamera implementiert
    *
    * @author bumaas
    * @version
    *   Version 2.50.1, 20.06.2018<br/>
    */

	IPSUtils_Include ('IPSComponentCam.class.php', 'IPSLibrary::app::core::IPSComponent::IPSComponentCam');

	class IPSComponentCam_Instar720pSeries extends IPSComponentCam {
		 ...
		 * @public
		 *
		 * Initialisierung eines IPSComponentCam_Instar720pSeries Objektes
		 *
		...
		public function Get_URLLiveStream($size=IPSCOMPONENTCAM_SIZE_MIDDLE) {
            ...
            $url = 'http://'.$this->ipAddress.'/cgi-bin/hi3510/mjpegstream.cgi?-chn='.$chn.'&-usr='.$this->username.'&-pwd='.$this->password;
		...
		public function Get_URLPicture($size=IPSCOMPONENTCAM_SIZE_MIDDLE) {
		    ...
		    switch ($size){
                case IPSCOMPONENTCAM_SIZE_SMALL:
                    $snapshot = 'auto2.jpg'; //Snapshot in 720p
                    break;
                case IPSCOMPONENTCAM_SIZE_MIDDLE:
                    $snapshot = 'auto.jpg';  //Snapshot in 352p
                    break;
                case IPSCOMPONENTCAM_SIZE_LARGE:
                    $snapshot = 'snap.jpg';  //Snapshot in 176p
                    break;
                default:
                    trigger_error('Unknown Size '.$size);
                    break;
		...
		public function Get_URL($urlType) {
			...
			switch ($urlType) {
                    case IPSCOMPONENTCAM_URL_MOVELEFT:
                        $url = $url.'/cgi-bin/hi3510/ptzctrl.cgi?-step=1&-act=left&-speed=40';
                        break;
                    case IPSCOMPONENTCAM_URL_MOVERIGHT:
                        $url = $url.'/cgi-bin/hi3510/ptzctrl.cgi?-step=1&-act=right&-speed=40';
                        break;
                    case IPSCOMPONENTCAM_URL_MOVEUP:
                        $url = $url.'/cgi-bin/hi3510/ptzctrl.cgi?-step=1&-act=up&-speed=40';
                        break;
                    case IPSCOMPONENTCAM_URL_MOVEDOWN:
                        $url = $url.'/cgi-bin/hi3510/ptzctrl.cgi?-step=1&-act=down&-speed=40';
                        break;
                    case IPSCOMPONENTCAM_URL_MOVEHOME:
                        $url = $url.'/cgi-bin/hi3510/ptzctrl.cgi?-step=1&-act=home&-speed=40';
                        break;
					case IPSCOMPONENTCAM_URL_PREDEFPOS1:
						$url = $url.'/cgi-bin/hi3510//param.cgi?cmd=preset&-act=goto&-number=0';
						break;
					case IPSCOMPONENTCAM_URL_PREDEFPOS2:
                        $url = $url.'/cgi-bin/hi3510//param.cgi?cmd=preset&-act=goto&-number=1';
						break;
					case IPSCOMPONENTCAM_URL_PREDEFPOS3:
                        $url = $url.'/cgi-bin/hi3510//param.cgi?cmd=preset&-act=goto&-number=2';
						break;
					case IPSCOMPONENTCAM_URL_PREDEFPOS4:
                        $url = $url.'/cgi-bin/hi3510//param.cgi?cmd=preset&-act=goto&-number=3';
						break;
