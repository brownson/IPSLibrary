<?
    ...
    */
	...
	class IPSComponentCam_K2000 extends IPSComponentCam {
		 ...
		 * @public
		 *
		 * Initialisierung eines IPSComponentCam_K2000 Objektes
		 *
		...
		public function Get_URLLiveStream($size=IPSCOMPONENTCAM_SIZE_MIDDLE) {
			$url = 'http://'.$this->username.':'.$this->password.'@'.$this->ipAddress.':'.$this->port.'/goform/stream?cmd=get&channel=4';				
			return $url;
		}
		...
		public function Get_URLPicture($size=IPSCOMPONENTCAM_SIZE_MIDDLE) {
			$url = 'http://'.$this->username.':'.$this->password.'@'.$this->ipAddress.':'.$this->port.'/snapshot/view0.jpg';
			return $url;
		}
		...
		public function Get_URL($urlType) {
			trigger_error('Diese Funktion ist für eine K2000 Kamera noch NICHT implementiert !!!');
		}
