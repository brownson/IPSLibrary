<?
	 ...
	 * @{
	 *
	 * @file          IPSWatering_Logging.inc.php
	 * @author        Andreas Brauneis
	 * @version
	 * Version 2.50.1, 11.03.2012<br/>
	 *
	 * Application Logging von IPSWatering
	 *
	 */

	 // ----------------------------------------------------------------------------------------------------------------------------
	function IPSWatering_LogActivate($CircleId, $Value, $Mode) {
		$CircleName = get_CirclyNameByID($CircleId);
		IPSLogger_Inf(__file__, bool2Activation($Value)." Watering ($Mode) for Circle '$CircleName'");
		if ($Value) {
			IPSWatering_Log("Aktiviere Beregnung ($Mode) für '$CircleName'");
		}
	}

	// ----------------------------------------------------------------------------------------------------------------------------
	function IPSWatering_LogNoActivationByRainfall($CircleId, $SensorLimit, $Rainfall) {
		$CircleName = get_CirclyNameByID($CircleId);
		IPSLogger_Dbg(__file__, "No Watering for Circle '$CircleName' during Rainfall of $Rainfall (Limit $SensorLimit)");
		IPSWatering_Log("Keine Bewässerung von $CircleName durch Regen (Sensor: $Rainfall)");
	}
