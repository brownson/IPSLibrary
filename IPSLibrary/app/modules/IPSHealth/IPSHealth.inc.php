...
//	IPSUtils_Include ("IPSHealth_IDs.inc.php",            	"IPSLibrary::app::modules::IPSHealth");


	// ----------------------------------------------------------------------------------------------------------------------------
	function IPSHealth_ChangeName($ControlId, $Value) {
	   ...
	   $parentId 		= get_CirclyIdByControlId($ControlId);
		...
		$CirclesId     = IPSUtil_ObjectIDByPath('Program.IPSLibrary.data.modules.IPSHealth.'.c_ZSUCircles);
	...
	function get_CircleConf($CircleId, $Value) {
		...
		set_AssoInteger('IPSHealth_StartTag', IPS_GetVariableProfile('IPSHealth_StartTag')['Associations'], $StartTag);
		set_AssoInteger('IPSHealth_StopTag',  IPS_GetVariableProfile('IPSHealth_StopTag')['Associations'], $StopTag);
		set_AssoInteger('IPSHealth_StartSensor', $Property[c_ZSUCircle.$Value][c_Property_StartSensoren], $StartAktiv);
		set_AssoInteger('IPSHealth_StopSensor', $Property[c_ZSUCircle.$Value][c_Property_StopSensoren],$StopAktiv);
		set_AssoInteger('IPSHealth_RunSensor', $Property[c_ZSUCircle.$Value][c_Property_RunSensoren],$RunAktiv);
	...
	// ----------------------------------------------------------------------------------------------------------------------------
	function IPSHealth_ChangeStartTag($ControlId, $Value) {
	   ...
	   $parentId 		= get_CirclyIdByControlId($ControlId);
		...
		if ($Value>0)	tgl_AssoValue('IPSHealth_StartTag', $Value);

		$ConfValue = get_AssoValue('IPSHealth_StartTag');
	...
	// ----------------------------------------------------------------------------------------------------------------------------
	function IPSHealth_ChangeStopTag($ControlId, $Value) {
		SetValueInteger ($ControlId,0);
	   ...
	   $parentId 		= get_CirclyIdByControlId($ControlId);
		...
		if ($Value>0)	tgl_AssoValue('IPSHealth_StopTag', $Value);

		$ConfValue = get_AssoValue('IPSHealth_StopTag');
	...
	// ----------------------------------------------------------------------------------------------------------------------------
	function IPSHealth_ChangeStartStunde($ControlId, $Value) {
		SetValueInteger ($ControlId,$Value);
	...
	// ----------------------------------------------------------------------------------------------------------------------------
	function IPSHealth_ChangeStopStunde($ControlId, $Value) {
		SetValueInteger ($ControlId,$Value);
	...
	// ----------------------------------------------------------------------------------------------------------------------------
	function IPSHealth_ChangeStartMinute($ControlId, $Value) {
		SetValueInteger ($ControlId,$Value);
	...
	// ----------------------------------------------------------------------------------------------------------------------------
	function IPSHealth_ChangeStopMinute($ControlId, $Value) {
		SetValueInteger ($ControlId,$Value);
	...
	// ----------------------------------------------------------------------------------------------------------------------------
	function IPSHealth_ChangeStartAktiv($ControlId, $Value) {
		SetValueInteger ($ControlId,0);
	   ...
	   $parentId 		= get_CirclyIdByControlId($ControlId);
		...
		if ($Value>0)	tgl_AssoValue('IPSHealth_StartSensor', $Value);

		$ConfValue = get_AssoValue('IPSHealth_StartSensor');
	...
	// ----------------------------------------------------------------------------------------------------------------------------
	function IPSHealth_ChangeStopAktiv($ControlId, $Value) {
		SetValueInteger ($ControlId,0);
	   ...
	   $parentId 		= get_CirclyIdByControlId($ControlId);
		...
		if ($Value>0)	tgl_AssoValue('IPSHealth_StopSensor', $Value);

		$ConfValue = get_AssoValue('IPSHealth_StopSensor');
	...
	// ----------------------------------------------------------------------------------------------------------------------------
	function IPSHealth_ChangeRunAktiv($ControlId, $Value) {
		SetValueInteger ($ControlId,0);
	   ...
	   $parentId 		= get_CirclyIdByControlId($ControlId);
		...
		if ($Value>0)	tgl_AssoValue('IPSHealth_RunSensor', $Value);

		$ConfValue = get_AssoValue('IPSHealth_RunSensor');
	...
	// ----------------------------------------------------------------------------------------------------------------------------
	function IPSHealth_ChangeInteger($ControlId, $Value) {
		SetValueInteger ($ControlId,$Value);
	...
	function set_TimerEvents($CircleId) {
		$StartTimeId  = IPSUtil_ObjectIDByPath('Program.IPSLibrary.app.modules.IPSHealth.IPSHealth_Timer.'.get_ControlType($CircleId).'-Start');
		$StopTimeId   = IPSUtil_ObjectIDByPath('Program.IPSLibrary.app.modules.IPSHealth.IPSHealth_Timer.'.get_ControlType($CircleId).'-Stop');
